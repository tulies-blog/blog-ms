...
const actions = {
  ...
  async list({ commit }, { payload }) {
    ...
    // 这边调用获取用户列表
    const resp = await commentService.listTopic(payload);
    if (resp.code === 0) {
  ...
  async delete({ commit }, { payload, callback }) {
    ...
    let count = 0;
    for (const id of ids) {
      const resp = await commentService.deleteTopic({ id });
      if (resp.code === 0) {
        count = count + 1;
      }
    }
  ...
  async changeStatus({ commit }, { payload, callback }) {
    ...
    let count = 0;
    for (const id of ids) {
      const resp = await commentService.changeTopicStatus(id, status);
      if (resp.code === 0) {
        count = count + 1;
      }
    }
